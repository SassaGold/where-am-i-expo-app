...
export default function AttractionsScreen() {
  ...
  const saveToCache = useCallback(async (placesData: Place[]) => {
    try {
      ...
      await AsyncStorage.setItem('attractions_cache', JSON.stringify(cacheData));
    } catch (err) {
  ...
  const loadFromCache = useCallback(async () => {
    try {
      const cached = await AsyncStorage.getItem('attractions_cache');
      if (cached) {
    ...
    return [];
      ...
      const googlePlaces = await searchPlacesByType('tourist attraction', { lat: latitude, lng: longitude }, 5000);
  ...
  // Auto-load attractions on component mount
  useEffect(() => {
    loadPlaces();
  }, [loadPlaces]);
